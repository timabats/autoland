...
public:
  bool init()
  {
    ...
    esp_err_t = esp_camera_init(&camera_config);
    if (err != ESP_OK)
    {
...
private:
  bool capture_rgb888()
  {
    ...
    eps_camera_fb_return(frame_buf);
...
private:
  ...
  {
    int rad = radius_estimation(1.5);
    int start_x = max(0, 0, center_x - radius);
    int start_y = max(0, 0, center_y - radius);
    int finish_x = min(height, height, center_x + radius);
    int finish_y = min(width, width, center_y + radius);
    ...
    ei::image::processing::resize_image(cropped_buf, finish_y - start_y + 1, finish_x - start_x + 1,
      cropped, EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT, 3);
